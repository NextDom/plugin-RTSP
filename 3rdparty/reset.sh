...
echo PPID supprimés :
echo `ps -afe | grep ffmpeg | awk '{print $3}'`
$sudo_prefix kill -9 `ps -afe | grep ffmpeg | grep rtsp | awk '{print $3}'`

echo PID supprimés :
echo `ps aux | grep ffmpeg | awk '{print $2}'`
$sudo_prefix kill -9 `ps aux | grep ffmpeg | grep rtsp | awk '{print $2}'`
...
echo `ps aux | grep ffmpeg | grep rtsp`
